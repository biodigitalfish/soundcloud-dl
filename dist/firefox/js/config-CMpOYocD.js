...
function isManifestV3() {
  try {
    ...
    logger$2.logWarn("Failed to detect manifest version:", e);
  }
  return false;
}
...
function usesDeclarativeNetRequestForModification() {
  return typeof chrome !== "undefined" && chrome.declarativeNetRequest && isManifestV3();
}
const RULE_ID_OAUTH = 1;
const RULE_ID_CLIENT_ID = 2;
async function setAuthHeaderRule(oauthToken) {
  if (!usesDeclarativeNetRequestForModification()) {
    logger$2.logDebug("Skipping declarativeNetRequest OAuth rule update: DNR not used for modification in this environment.");
    return;
  }
  const rulesToAdd = [];
  ...
  try {
    if (typeof chrome !== "undefined" && chrome.declarativeNetRequest) {
      ...
      logger$2.logInfo(`OAuth DNR rule updated. Token: ${oauthToken ? "SET" : "REMOVED"}`);
    } else {
      logger$2.logError("Cannot update DNR rules: chrome.declarativeNetRequest is not defined.");
    }
  } catch (error) {
    logger$2.logError("Failed to update DNR rules for OAuth token:", error);
  }
}
async function setClientIdRule(clientId) {
  if (!usesDeclarativeNetRequestForModification()) {
    logger$2.logDebug("Skipping declarativeNetRequest ClientID rule update: DNR not used for modification in this environment.");
    return;
  }
  const rulesToAdd = [];
  ...
  if (clientId) {
    rulesToAdd.push({
      ...
      condition: {
        ...
        // Potentially limit to soundcloud.com if needed
  ...
  try {
    if (typeof chrome !== "undefined" && chrome.declarativeNetRequest) {
      ...
      logger$2.logInfo(`Client_id DNR rule updated. ClientID: ${clientId ? "SET" : "REMOVED"}`);
    } else {
      logger$2.logError("Cannot update DNR rules: chrome.declarativeNetRequest is not defined.");
    }
  } catch (error) {
    logger$2.logError("Failed to update DNR rules for client_id:", error);
  }
}
async function createURLFromBlob(blob) {
  if (typeof URL !== "undefined" && typeof URL.createObjectURL === "function") {
    try {
      logger$2.logDebug("Using URL.createObjectURL");
      return URL.createObjectURL(blob);
    } catch (e) {
      logger$2.logWarn("URL.createObjectURL failed:", e);
    }
  }
  logger$2.logInfo("Falling back to FileReader.readAsDataURL");
  try {
    ...
    logger$2.logError("Both URL creation methods failed:", e);
...
function revokeURL(url) {
  if (url && url.startsWith("blob:")) {
    try {
      ...
      logger$2.logDebug("Object URL revoked");
    } catch (e) {
      logger$2.logWarn("Failed to revoke object URL:", e);
    }
  }
}
function eraseDownloadHistoryEntry(filenameRegex) {
  if (typeof chrome !== "undefined" && chrome.downloads && chrome.downloads.erase) {
    const query = {
      filenameRegex,
      state: "complete"
      // Only target completed downloads
    };
    chrome.downloads.erase(query, (erasedIds) => {
      if (erasedIds && erasedIds.length > 0) {
        logger$2.logInfo(`Force redownload: Removed ${erasedIds.length} matching entries from browser download history.`);
      } else {
        logger$2.logDebug("Force redownload: No matching entries found in browser download history to erase.");
      }
    });
  } else {
    logger$2.logDebug("Skipping browser download history erase: chrome.downloads.erase is not available.");
  }
}
function determineIfUrlIsSet(url, initialIsSet) {
  let finalIsSet = initialIsSet;
  if (!finalIsSet && typeof browser !== "undefined" && url) {
    if (url.includes("/sets/") || url.includes("/albums/")) {
      logger$2.logDebug(`[BrowserUtils] Firefox detected, forcing isSet=true for URL: ${url}`);
      finalIsSet = true;
    }
  }
  return finalIsSet;
...
const onBeforeSendHeaders = (callback, urls, extraInfos) => {
  ...
  if (typeof browser !== "undefined" && browser.webRequest && browser.webRequest.onBeforeSendHeaders) {
    browser.webRequest.onBeforeSendHeaders.addListener(callback, filter, extraInfos);
    ...
    logger$1.logError("Browser does not support webRequest.onBeforeSendHeaders");
  }
};
const onBeforeRequest = (callback, urls, extraInfos) => {
  ...
  if (typeof browser !== "undefined" && browser.webRequest && browser.webRequest.onBeforeRequest) {
    browser.webRequest.onBeforeRequest.addListener(callback, filter, extraInfos);
    ...
    logger$1.logError("Browser does not support webRequest.onBeforeRequest");
  }
};
const downloadToFile = (url, filename, saveAs) => {
  ...
  return new Promise((resolve, reject) => {
    let downloadIdInternal;
    let onChangedHandlerInstance;
    const createAndRegisterOnChangedHandler = (browserOrChrome, currentDownloadId) => {
      const handler = (delta) => {
        if (delta.id === currentDownloadId) {
          if (delta.state?.current === "complete") {
            ...
            revokeURL(url);
            logger$1.logInfo(`Download ${currentDownloadId} completed and cleaned up.`);
            ...
            logger$1.logWarn(`Download ${currentDownloadId} failed or interrupted post-initiation: ${errorReason}`);
            revokeURL(url);
          } else if (delta.error && (delta.error.current === "USER_CANCELED" || delta.error.current === "DOWNLOAD_CANCELLED_BY_USER")) {
            logger$1.logInfo(`Download ${currentDownloadId} was canceled by the user.`);
            if (onChangedHandlerInstance) browserOrChrome.downloads.onChanged.removeListener(onChangedHandlerInstance);
            revokeURL(url);
          }
        }
      };
      onChangedHandlerInstance = handler;
      browserOrChrome.downloads.onChanged.addListener(onChangedHandlerInstance);
    };
    if (typeof browser !== "undefined" && browser.downloads) {
      browser.downloads.download(downloadOptions).then((id) => {
        if (id === void 0) {
          ...
          logger$1.logError("Firefox download initiation error (no id):", errMsg);
          revokeURL(url);
        ...
        resolve(downloadIdInternal);
      }).catch((err) => {
        const errMsg = err?.message || String(err) || "browser.downloads.download call failed";
        logger$1.logError("Firefox browser.downloads.download promise rejected:", errMsg);
        revokeURL(url);
        reject(new Error(`Firefox download failed: ${errMsg}`));
      });
    } else if (typeof chrome !== "undefined" && chrome.downloads) {
      chrome.downloads.download(downloadOptions, (idCallbackValue) => {
        if (chrome.runtime.lastError) {
          const errMsg = chrome.runtime.lastError.message || "Download initiation failed in Chrome (callback error)";
          logger$1.logError("Chrome download initiation error (lastError):", errMsg);
          revokeURL(url);
          reject(new Error(errMsg));
          return;
        }
        if (idCallbackValue === void 0) {
          const errMsg = "Download initiation failed: no downloadId returned (Chrome).";
          logger$1.logError(errMsg);
          revokeURL(url);
          reject(new Error(errMsg));
          return;
        }
...
const onPageActionClicked = (callback) => {
  if (typeof browser !== "undefined" && browser.pageAction) {
    ...
    logger$1.logError("Browser does not support pageAction.onClicked or action.onClicked");
  }
};
const openOptionsPage = () => {
  if (typeof browser !== "undefined" && browser.runtime && browser.runtime.openOptionsPage) {
    ...
    logger$1.logError("Browser does not support runtime.openOptionsPage");
  }
};
const onStorageChanged = (callback) => {
  if (typeof browser !== "undefined" && browser.storage && browser.storage.onChanged) {
    ...
    logger$1.logError("Browser does not support storage.onChanged");
  }
};
...
const getExtensionManifest = () => {
  if (typeof browser !== "undefined" && browser.runtime && browser.runtime.getManifest) {
    ...
    logger$1.logError("Browser does not support runtime.getManifest");
    return null;
  }
};
const getPathFromExtensionFile = (relativePath) => {
  if (typeof browser !== "undefined" && browser.runtime && browser.runtime.getURL) {
    ...
    logger$1.logError("Browser does not support runtime.getURL");
    return null;
  }
};
const searchDownloads = (query) => {
  if (typeof browser !== "undefined" && browser.downloads && browser.downloads.search) {
    return browser.downloads.search(query).then((items) => {
      return items.map((item) => ({
        ...item,
        finalUrl: item.url
        // Add finalUrl, using url as a fallback for compatibility
      }));
    });
    ...
    logger$1.logError("Browser does not support downloads.search");
...
const fixed = {};
let regexCache = /* @__PURE__ */ Object.create(null);
let patternCache = /* @__PURE__ */ Object.create(null);
const tokens = [];
...
function requireCategories() {
  if (hasRequiredCategories) return categories$1;
...
var categoriesExports = requireCategories();
const categories = /* @__PURE__ */ getDefaultExportFromCjs(categoriesExports);
/*!
...
const scripts = /* @__PURE__ */ getDefaultExportFromCjs(scriptsExports);
/*!
...
async function storeConfigValue(key, value) {
  ...
  try {
    ...
    if (entry.onChanged) entry.onChanged(value);
  } catch (_error) {
...
async function loadConfigValues(keys) {
...
async function loadConfiguration(monitorStorage = false) {
