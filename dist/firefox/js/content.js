    ...
    if (isCompletionMessageEvaluation) {
      ...
      if (activeIdsForCompletionLogic.length === 1) {
        ...
        if (mostRecentId) {
          ...
          logger.logWarn(`[HANDLE_MSG_FROM_BG] Found ${activeIdsForCompletionLogic.length} active downloads, but couldn't match generic completion message by timestamp.`);
        }
        ...
        logger.logWarn("[HANDLE_MSG_FROM_BG] No active downloads to match generic completion message to.");
      }
    }
    if (!finalDownloadId) {
      if (currentActiveDownloads.length === 0 && isMinimalMessage) {
        ...
        if (status === void 0) {
          ...
          if (completed === void 0) {
            ...
            if (!error) {
              ...
              logger.logWarn(`[CS_DEBUG_ACK_FAIL_FINAL_BLOCK] !message.error FAILED. Error: ${error}`);
            }
          } else {
            logger.logWarn(`[CS_DEBUG_ACK_FAIL_FINAL_BLOCK] message.completed FAILED. Was: ${completed}`);
          }
        } else {
          logger.logWarn(`[CS_DEBUG_ACK_FAIL_FINAL_BLOCK] message.status FAILED. Was: ${status}`);
        }
      } else {
        logger.logWarn(`[CS_DEBUG_ACK_FAIL_FINAL_BLOCK] message.progress FAILED. Was: ${progress}`);
      }
    } else {
      logger.logWarn(`[CS_DEBUG_ACK_FAIL_FINAL_BLOCK] currentState was NOT Preparing. Was: ${currentState}`);
    }
  }
  if ((messagePayload.success === true && !error || messagePayload.error === "" && messagePayload.success === void 0) && originalIdFromPayload === finalDownloadId && currentState === "Preparing" && progress === void 0 && status === void 0 && completed === void 0) {
